...
const SpareParts = () => {
    ...
    const handleOrder = (item) => {
        // navigate to AddToCart page for this item
        navigate(`/add-to-cart/${item._id}`);
    };
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
                        ...
                        <div className={`${showFilters ? 'block' : 'hidden'} lg:block`}>
                            {brands.map(brandName => (
                                <label key={brandName} className="flex items-center space-x-3 mb-3 cursor-pointer">
                                    <input
                                        type="checkbox"
                    ...
                    {filteredParts.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {filteredParts.map(item => (
                                <div key={item._id} className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow duration-200 border border-gray-100 overflow-hidden">
                                    ...
                                    <div className="p-6">
                                        <div className="flex items-center gap-2 mb-3">
                                            <span className={`inline-block w-3 h-3 rounded-full ${item.quantity > 0 ? 'bg-green-500' : 'bg-red-500'}`}></span>
                                            <span className={`text-sm ${item.quantity > 0 ? 'text-green-600' : 'text-red-600'}`}>
                                                {item.quantity > 0 ? 'In Stock' : 'Out of Stock'}
                                            </span>
                                        </div>
                                        <h3 className="text-xl font-semibold text-gray-800 mb-2">{item.name}</h3>
                                        <p className="text-sm text-gray-600 mb-1">{item.brand} - {item.modelNumber}</p>
                                        ...
                                        <button
                                            ...
                                            className="mt-4 w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-lg disabled:bg-gray-400 transition"
                                        >
